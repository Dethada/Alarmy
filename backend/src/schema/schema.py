...
class Query(graphene.ObjectType):
    ...
    person_alert = graphene.Field(PersonAlertType, cid=graphene.Int(required=True))
    ...
    def resolve_all_temp(self, info, duration=''):
        if duration == '24H':
            ...
            df = analysis_helper(Temperature.query.filter(Temperature.capture_time > since).statement, 'H')
            result = [CustomTempType(capture_time=time, value=value) for time, value in zip(df.index, df['value'])]
        elif duration == '7D':
            since = datetime.now() - timedelta(days=7)
            df = analysis_helper(Temperature.query.filter(Temperature.capture_time > since).statement, '4H')
            result = [CustomTempType(capture_time=time, value=value) for time, value in zip(df.index, df['value'])]
        elif duration == '30D':
            since = datetime.now() - timedelta(days=30)
            df = analysis_helper(Temperature.query.filter(Temperature.capture_time > since).statement, 'D')
            result = [CustomTempType(capture_time=time, value=value) for time, value in zip(df.index, df['value'])]
        elif duration == 'ALL':
            df = analysis_helper(Temperature.query.statement, '2W')
            result = [CustomTempType(capture_time=time, value=value) for time, value in zip(df.index, df['value'])]
        else:
            result = [CustomTempType(capture_time=x.capture_time, value=x.value) for x in Temperature.query.order_by(Temperature.capture_time.desc()).limit(20)]
        return result
    ...
    def resolve_all_gas(self, info, duration=''):
        ...
        if duration == '24H':
            since = datetime.now() - timedelta(hours=24)
            df = analysis_helper(Gas.query.filter(Gas.capture_time > since).statement, 'H')
            for row in df.itertuples():
                result.append(CustomGasType(capture_time=row.Index, lpg=row.lpg, co=row.co, smoke=row.smoke))
        elif duration == '7D':
            since = datetime.now() - timedelta(days=7)
            df = analysis_helper(Gas.query.filter(Gas.capture_time > since).statement, '4H')
            for row in df.itertuples():
                result.append(CustomGasType(capture_time=row.Index, lpg=row.lpg, co=row.co, smoke=row.smoke))
        elif duration == '30D':
            since = datetime.now() - timedelta(days=30)
            df = analysis_helper(Gas.query.filter(Gas.capture_time > since).statement, 'D')
            for row in df.itertuples():
                result.append(CustomGasType(capture_time=row.Index, lpg=row.lpg, co=row.co, smoke=row.smoke))
        elif duration == 'ALL':
            ...
            result = [CustomGasType(capture_time=x.capture_time, lpg=x.lpg, co=x.co, smoke=x.smoke) for x in Gas.query.order_by(Gas.capture_time.desc()).limit(20)]
        return result
